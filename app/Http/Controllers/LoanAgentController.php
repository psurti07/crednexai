...
class LoanAgentController extends Controller
{
        ...
        if ($userDetails) {
            ...
            if ($userDetails->paymentid != NULL && $userDetails->isActive == 1 && $userDetails->isDelete == 0) {
                ...
                if ($userDetails->isCustomer == 1) {
                    ...
                    return array('type' => 'SUCCESS', 'message' => 'You are already a customer. Please login to your customer portal.', 'redirect' => route('customer.login'));
                } else {
    ...
    {
        $meta = selfApplyMeta();
        $products = Product::where('productslug', 'la-offer-2')->first();
        if ($products->inOffer == 1) {
            $productData = array(
                'inOffer' => $products->inOffer,
                'amount' => $products->amount,
