...
use App\Jobs\SendSACustomerSmsJob;
use App\Models\UserRegistration;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\DB;

class SACustomerSmsServices
{
    public function run()
    {
        try {
            ...
            $schedules = config('remarketing.saCustomerSms');
            
            $msgTemplate = DB::table('sms_list')
                ->where('type', 1)
                ->where('slug', 'sales_cycle_days')
                ->first()
                ->message ?? '';
